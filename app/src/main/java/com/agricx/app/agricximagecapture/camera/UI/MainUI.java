...
public class MainUI {
    ...
    private void layoutUI(boolean popup_container_only) {
		...
		if( !popup_container_only )
		{
			...
			view = main_activity.findViewById(R.id.acceptDeclineContainer);
			layoutParams = (RelativeLayout.LayoutParams)view.getLayoutParams();
			layoutParams.addRule(align_parent_left, 0);
			layoutParams.addRule(align_parent_right, RelativeLayout.TRUE);
			view.setLayoutParams(layoutParams);

			view = main_activity.findViewById(R.id.acceptPicture);
			setViewRotation(view, ui_rotation);

			view = main_activity.findViewById(R.id.declinePicture);
			setViewRotation(view, ui_rotation);
